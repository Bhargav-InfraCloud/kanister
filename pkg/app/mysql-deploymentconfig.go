...
func (mdep *MysqlDepConfig) Install(ctx context.Context, namespace string) error {
	...
	return fmt.Errorf("error installing app %s on openshift cluster: %w", mdep.name, err)
}
...
func (mdep *MysqlDepConfig) Insert(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("error while inserting the data into msyql deployment config database: %s: %w", stderr, err)
	}
...
func (mdep *MysqlDepConfig) execCommand(ctx context.Context, command []string) (string, string, error) {
	...
	if err != nil {
		return stdout, stderr, fmt.Errorf("error executing command in the pod: %w", err)
	}
